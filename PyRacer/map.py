...
class Map():
    ...
    def draw_map(self):
        ...
        while y < self.mid_h:
            ...
            while x < self.game.DISPLAY_W:
                ...
                if RightClip <= x < RightGrass:
                    pygame.draw.rect(self.game.display, clip_color, (x, nRow, 8, 8))
                if RightGrass <= x < self.game.DISPLAY_W:
                    pygame.draw.rect(self.game.display, grass_color, (x, nRow, 8, 8))
                x += 8
            y += 8
    ...
    def draw_stats(self):
        ...
        if self.game.complete:
            ...
            self.game.draw_text('Best Time:' + str(round(self.game.best_time, 2)), (0, 0, 255), self.mid_w * .5,
                                self.mid_h + 40)
